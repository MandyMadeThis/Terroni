...
jQuery.validator.addMethod("alphadotdash", function(value, element) {
	return this.optional(element) || /^([a-z\s\-\.])+$/i.test(value);
}, "Letters, hyphens, or periods only please");
...
// method to check for 'advanced' punctuation
jQuery.validator.addMethod("lettersNumbersWithAdvancedPunc", function(value, element){
    return this.optional(element) || /^[a-z0-9\!-.:;\?()'ÀàÁáÂâÄäÆæÉéÈèÊêËëÎîÏïÔôŒœÒòÔôÖöŸÿÇçÙÛÜùûü«»º\s]+$/i.test(value);
}, "Letters, numbers, or punctuation only please");

// Method to check for profanity
jQuery.validator.addMethod("profanity", function(value, element) {
	var passedProfanityTest = false;
    $.post(HOST_URL + '/system/scripts/checkProfanity.php', { t: value }, function(resp) {
		if(resp === false) {
			passedProfanityTest = true;
		}
    });
    return passedProfanityTest;
}, "Sorry, that name may offend some audiences.");